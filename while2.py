...
class Semantics:
    ...
    def left(self, ast):
        if isinstance(ast, list):
            return {'type': 'left', 'id': ast[0], 'val': ast[2]}
        else:
            return ast
    ...
    def tableau(self, ast):
        def decompose(ast):
            if isinstance(ast, tuple):
                return decompose(ast[1]) + [ast[2]]
            else:
                return [ast]
        return {'type': 'tableau', 'list': decompose(ast[0])}
...
def pprint_expr(expr_ast):
    if expr_ast['type'] == 'opbin':
        return "%s %s %s" % (pprint_expr(expr_ast['gauche']),
                             expr_ast['op'],
                             pprint_expr(expr_ast['droit']))
    elif expr_ast['type'] == 'tableau':
        variables = ", ".join([pprint_expr(x) for x in expr_ast['list']])
        return "{ %s }" % variables
    elif expr_ast['type'] == 'constant':
        return str(expr_ast['val'])
    elif expr_ast['type'] == 'left':
        return pprint_expr(expr_ast['id']) + '[' + pprint_expr(expr_ast['val']) + ']'
    else:
        return expr_ast['id']
...
def pprint_com(com_ast):
    if com_ast['type'] == 'aff':
        return "%s = %s ; " % (pprint_expr(com_ast['lhs']), pprint_expr(com_ast['rhs']))
        ...
        return 'if(%s){\n %s\n }' % (pprint_expr(com_ast['expr']),
                                     pprint_com(com_ast['body']))
    elif com_ast['type'] == 'dec':
        return '%s ;' %(pprint_expr(com_ast['var']))
    else:
